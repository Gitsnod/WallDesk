...
5. **视频壁纸配频谱（新）**：此前视频壁纸一律走静音路径，桌面频谱对视频永远没反应。
   ...
   音量仍交给系统音量控制。开了频谱却发现没反应时，把「设置 → 视频 → 音量」拉到 0 以上即可。
...
- **场景预设（V4.7）**：壁纸库页的「场景预设」一行可应用 / 保存 / 删除；设置页与托盘右键各有入口。
  预设文件是数据目录下的 `presets.json`，可直接编辑或随配置一起拷走
- **在线图源（V4.7）**：壁纸库页「在线添加」一行可粘贴图片网址（多个地址换行分隔）或抓取 Bing 每日一图；
  下载落在数据目录的 `downloads/`
- **按分辨率 / 朝向分组（V4.7）**：检索行的筛选器支持横向 / 纵向 / 方形与 SD…4K 五档，
  排序可选「按分辨率」。悬停任意一项可看到尺寸与档位
- **视频配频谱（V4.7）**：设置 →「视频 → 音量」调到 0 以上，视频声音进入系统混音，
