...
**当前版本：4.6.0（V4 发行版系列的第六次迭代）**。V4 补齐了「发给别人用」所需的一切：
...
1. **去顶部工具条，功能全进设置页**：库管理（添加 / 移除 / 清空）与播放控制（应用 / 下一张 / 暂停 /
   停止 / 重新挂载）统一收进「设置 → 操作」；原「工具」页并入设置（外观主题、打开数据目录 /
   日志、关于、退出），左侧导航只保留「壁纸库 / 设置 / 多屏」。
2. **修复「启动时恢复上次壁纸」失效**：关闭窗口隐藏到托盘前也会保存设置，且恢复开关接入了
   `toggled` 保存信号，勾选状态会真正落盘。
...
产物：`dist\WallDesk-4.6.0-win64.zip`（约 25–60 MB，取决于是否内嵌 VLC）。
...
git commit -m "WallDesk v4.6.0: 图片/视频壁纸，内嵌 VLC，便携模式"
...
```bash
# 打标签
git tag -a v4.6.0 -m "WallDesk 4.6.0"
git push origin v4.6.0
```

然后到 GitHub 仓库页 → **Releases** → **Draft a new release**：
- Tag 选 `v4.6.0`，标题 `WallDesk 4.6.0`
- 上传 `dist\WallDesk-4.6.0-win64.zip` 和（可选）`WallDesk-4.6.0-setup.exe`
...
```bash
gh release create v4.6.0 dist\WallDesk-4.6.0-win64.zip dist\WallDesk-4.6.0-setup.exe ^
  --title "WallDesk 4.6.0" --notes "见 README 九、版本变更"
```
